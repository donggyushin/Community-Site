...
<table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col"></th>
      <th scope="col">Board View Table</th>
      <th scope="col"></th>
      <th scope="col"></th>
      <th></th>
    </tr>
  ...
  <tbody>
    <tr>
      <th scope="row" style="width: 70px;">Num</th>
      <td>Title</td>
      <td>Writer</td>
      <td style="width: 100px;">Date</td>
      <td style="width: 70px;">Hit</td>
    </tr>
    <%
    	ArrayList<BoardDTO> boardList = new BoardDAO().getListByPageNumber(pageNumber);
    	for(int i = 0 ; i < boardList.size(); i ++){
	%>
	<tr>
      <th scope="row" style="width: 70px;"><%=boardList.get(i).getBoardID() %></th>
      <td><a href="boardShow.jsp?boardID=<%=boardList.get(i).getBoardID()%>"><%=boardList.get(i).getBoardTitle() %></a></td>
      <td><%=boardList.get(i).getUserID() %></td>
      <td style="width: 180px;"><%=boardList.get(i).getBoardDate().substring(0, 10) %></td>
      <td style="width: 70px;"><%=boardList.get(i).getBoardHit() %></td>
    </tr>
	<%    		
    	}
    %>
    ...
    <tr>
      ...
      <td></td>
      <td></td>
      <td><a href="boardWrite.jsp" class="btn btn-primary">Write</a></td>
    </tr>
  </tbody>
</table>  
</div>
...
<div class="container">
  ...
  <%
    if(pageNumber > 1){
    %>
      <li class="page-item"><a class="page-link" href="boardView.jsp?pageNumber=<%=pageNumber - 1%>">Previous</a></li>
    
    <%  
    }
  %>
    <%
      int result = new BoardDAO().checkNextPage(pageNumber + 1);
      if(result == 1){
      %>
        <li class="page-item"><a class="page-link" href="boardView.jsp?pageNumber=<%=pageNumber + 1%>">Next</a></li>
      <%  
      }
    %>
